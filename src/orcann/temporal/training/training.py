"""Cross-indicator training: fit the temporal rate head on public ground truth
and evaluate it leave-one-indicator-out (LOIO) before any organoid data. Runs
through one interface, ``GroundTruthNeuron`` (real CASCADE via ``load_cascade_mat``
or a synthetic stand-in). See docs/temporal/training.md.
"""
...
def _events_to_seconds(ap: np.ndarray, fluo_time: np.ndarray) -> np.ndarray:
    """Coerce events_AP to seconds, auto-detecting the storage scale against the
    fluorescence-time span (logged). See docs/temporal/training.md."""
    if len(ap) == 0:
...
def preprocess(neuron: GroundTruthNeuron, dst_fs: float, target_noise_sd: float,
    ...
    """One neuron -> (standardised trace at target rate, smoothed rate target):
    resample → add noise → standardise. See docs/temporal/training.md."""
...
def _median_corr(pred: torch.Tensor, target: torch.Tensor) -> float:
    """Median Pearson correlation over windows that contain firing (a constant
    silent-neuron target has undefined correlation, so it is excluded from the
    metric but still trains through MSE)."""
...
def train_final(
    ...
    """Train one deployable rate model on all supplied neurons (no held-out fold);
    this is the model used for inference. See docs/temporal/training.md."""
    rng = np.random.default_rng(seed)
    Xtr, Ytr = windowize([preprocess(n, dst_fs, target_noise_sd, smooth_s, rng)
