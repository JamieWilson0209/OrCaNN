...
Both are $\nabla^2 G$. LoG sweeps a **spatial** scale $\sigma$ for blobs of varying radius; the Ricker CWT sweeps a **temporal** scale $a$ for transients of varying duration. A blob of radius $r$ is matched at $\sigma=r/\sqrt{2}$; a transient at the $a$ whose Fourier period equals its width.
...
* **`ParametricLoG2d`** generates $K$ kernels from learnable $\log\sigma$: $-\sigma^2\nabla^2 G_\sigma$ (bright blob → positive), scale-normalised (Lindeberg) and demeaned (DC rejection). Only the scales are free. Shared across all channels.
...
$$\psi_\theta(t)\ =\ \cos\theta\,R(t)\ +\ \sin\theta\,D(t),\qquad R\ \propto\ \big(1-\tfrac{t^2}{a^2}\big)e^{-t^2/2a^2}\ (-\nabla^2 G),\quad D\ \propto\ \tfrac{t}{a}\,e^{-t^2/2a^2}\ (\nabla G)$$
...
2.  **normalise** — divide by the robust noise $\sigma$ (MAD of differences) → SNR units.
...
* **Target:** bin spike times to 2 Hz and Gaussian-smooth ($\sigma \approx 1$ s) → per-bin rate. Never discrete sub-bin spikes.
...
$$Y\ \xrightarrow{\text{Stage 1 (scattering detector, raw movie)}}\ \text{cellness} \ \xrightarrow{\text{peaks + nearest-centroid, activity-gated}}\ A_i \ \xrightarrow{\ C_i=\frac{\sum_x A_i(x)Y(x,t)}{\sum_x A_i(x)}\ }\ C_i \ \xrightarrow{\text{standardize\_trace}}\ \xrightarrow{\text{Stage 2}}\ \hat{r}_i,\ \hat{\tau}_i$$
...
8.  **$\Delta F/F$ unified** into one `standardize_trace` across training and inference (noise-invariance by normalisation).
...
**Eddie deployment** (`eddie/`): `config.sh` (all cluster-specific values), `setup_eddie.sh` (conda prefix env + CUDA-matched torch via `CUDA_BUILD`, default `cu121`; `PYTHONNOUSERSITE`), `make_workspace.sh`, and SGE jobs `check_gpu.sh`, `train_spatial.sh` (GPU), `train_temporal_loio.sh` (CPU), `infer_array.sh` (CPU array over a manifest).
...
* **Bounded memory:** recordings stream from disk one at a time; per-chunk LoG response ~101 MB at $512^2$ (ample on an 80 GB A100). Patch training (default 128 px, 16 patches/recording, 256 energy frames) gives field coverage with bounded cost.
...
**Status:** every module smoke, every runner self-test, all shell scripts, and a realistic-scale forward+backward pass clean; edge cases (flat/silent trace, zero detection) produce no NaN/crash. **No real recording trained on or evaluated yet.**
...
* **Temporal domain shift.** Public ground truth is GCaMP/OGB, 2-photon, in-vivo cortical — not Fluo-4 widefield organoid. Cross-indicator training mitigates but does not erase this; LOIO is the early detector, synthetic the fallback. The temporal stage's honest claim is unification/speed/learned-gating, with the synthetic-to-real gap as the standing risk; the spatial stage earns a real supervised number.
* **Duration ≠ $\tau$.** Reported durations are wavelet characteristic timescales — an inflated, roughly constant multiple of the decay constant. Ordering/separation faithful; absolute seconds not.
...
* **Synthetic only.** Every quantitative result so far is from synthetic stand-ins testing mechanism. Accuracy claims await the real harnesses.
