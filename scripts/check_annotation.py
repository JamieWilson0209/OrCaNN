...
from orcann.spatial import _load_annotation
from orcann.pipeline.figures import max_projection_figure


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--movie", required=True)
    ap.add_argument("--annotation", required=True, help="ImageJ RoiSet.zip")
    ap.add_argument("--out", default="annotation_check.png")
    a = ap.parse_args()

    movie = _load_movie(a.movie)
    H, W = movie.shape[1:]
    cents, radii = _load_annotation(a.annotation, (H, W))
